...
final class ProcessJobsCompositionTest extends TestCase
{
    ...
    public function testConfiguredTenMegabyteLimitIsPassedToFetchHandler(): void
    {
        ...
        try {
            $process = proc_open([PHP_BINARY, '-d', 'auto_prepend_file=' . $prepend, dirname(__DIR__, 2) . '/bin/process-jobs.php', '--limit=1', '--time-budget=5'], [1 => ['pipe', 'w'], 2 => ['pipe', 'w']], $pipes, null, $environment, ['bypass_shell' => true]);
            self::assertIsResource($process);
            $stdout = stream_get_contents($pipes[1]);
            $stderr = stream_get_contents($pipes[2]);
            fclose($pipes[1]); fclose($pipes[2]);
            ...
            self::assertSame('10485760', file_get_contents($capture));
        } finally {
            @unlink($prepend);
            @unlink($capture);
        }

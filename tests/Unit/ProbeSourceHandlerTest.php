...
{
    public function testSchedulesAiInsteadOfPublishingLegacyReadyAfterProbe(): void
    {
        ...
        $scheduler = new RecordingAiPipelineScheduler();
        $handler = new ProbeSourceHandler(
            $projects,
            $sources,
        ...
        self::assertSame([['probing', 70]], $projects->states);
    }

    public function testReadySourceReplaySchedulesAiWithoutInspectingAgain(): void
    {
        $projects = new HandlerProjectRepository();
        $sources = new HandlerSourceRepository(new ProjectSource(10, 4, 'local', 'imports/video.mp4', 'video/mp4'));
        $sources->readyDetails = ['source' => new ProjectSource(10, 4, 'local', 'imports/video.mp4', 'video/mp4'), 'duration_seconds' => 126];
        $scheduler = new RecordingAiPipelineScheduler();
        $processor = new HandlerProcessor(null);
        $handler = new ProbeSourceHandler($projects, $sources, $processor, new HandlerProcessingEffectGuard(new HandlerPdo()), $scheduler);
    ...
    public function testDefersWithoutFailingWhenTheProcessorCapabilityIsUnavailable(): void
    {
        $projects = new HandlerProjectRepository();
        $sources = new HandlerSourceRepository(new ProjectSource(10, 4, 'local', 'imports/video.mp4', 'video/mp4'));
        ...
        self::assertSame([['probing', 70], ['queued', 70]], $projects->states);
    }

    public function testProcessTimeoutStillConsumesANormalRetryAttempt(): void
    {
        $projects = new HandlerProjectRepository();
        $sources = new HandlerSourceRepository(new ProjectSource(10, 4, 'local', 'imports/video.mp4', 'video/mp4'));
        $handler = new ProbeSourceHandler($projects, $sources, new HandlerProcessor(null, new ProcessExecutionException('process_timeout')), new HandlerProcessingEffectGuard(new HandlerPdo()));

        $outcome = $handler->handle($this->job());

        self::assertSame('retry', $outcome->status());
        self::assertSame('process_timeout', $outcome->code());
        self::assertSame([], $sources->failedIds);
    }
...
final class RecordingAiPipelineScheduler implements AiPipelineScheduler
{
    ...
    public function schedule(int $projectId, int $sourceId, int $durationSeconds): AiAnalysisReceipt
    {
        ...
        return new AiAnalysisReceipt(1, 1, 'queued', true);
    }
}

...
final class DeploymentReleaseBuilderTest extends TestCase
{
 ...
 public function testBuildsAllowlistedArchiveWithVerifiedManifestAndNoSecrets(): void { $output=sys_get_temp_dir().'/cliplab-'.bin2hex(random_bytes(6)).'.zip'; try{(new ReleaseBuilder($this->root))->build($output); $zip=new ZipArchive(); self::assertTrue($zip->open($output)); self::assertNotFalse($zip->locateName('app/a.php')); self::assertNotFalse($zip->locateName('public/.htaccess')); self::assertNotFalse($zip->locateName('.htaccess')); self::assertNotFalse($zip->locateName('vendor/autoload.php')); self::assertFalse($zip->locateName('.env')); self::assertFalse($zip->locateName('storage/media.mp4')); $manifest=json_decode((string)$zip->getFromName('release-manifest.json'),true,512,JSON_THROW_ON_ERROR); self::assertSame(hash('sha256','<?php'),$manifest['files']['app/a.php']); $zip->close();}finally{@unlink($output);} }
